  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg w-[95vw] max-h-[90vh] overflow-y-auto bg-card">
        <DialogHeader><DialogTitle className="font-black">{expense ? "Edit Expense" : "Add Expense"}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label className="text-[11px]">Title *</Label><Input value={form.title} onChange={e => set("title", e.target.value)} placeholder="e.g. Monthly Rent" /></div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label className="text-[11px]">Amount (₹) *</Label><Input type="number" min={0} value={form.amount} onChange={e => set("amount", Number(e.target.value))} placeholder="0.00" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-[11px]">Paid To</Label><Input value={form.paid_to} onChange={e => set("paid_to", e.target.value)} placeholder="Vendor / person" /></div>
            <div><Label className="text-[11px]">Reference No.</Label><Input value={form.reference_no} onChange={e => set("reference_no", e.target.value)} placeholder="Optional" /></div>
          </div>
          <div><Label className="text-[11px]">GST Amount (₹)</Label><Input type="number" min={0} value={form.gst_amount} onChange={e => set("gst_amount", Number(e.target.value))} /></div>
          <div><Label className="text-[11px]">Notes</Label><Input value={form.notes} onChange={e => set("notes", e.target.value)} placeholder="Optional notes" /></div>
          <div className="flex gap-2 pt-2 border-t border-border">
            <Button onClick={() => onSave(form)} disabled={!form.title || !form.amount || !form.date} className="gold-gradient text-black font-bold">💾 Save</Button>
    ...
    <div className="animate-fade-up space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Track all business expenses & operational costs</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Total Expenses", value: fmtINR(totalAmount), color: "text-red-400", icon: "💸" },
          { label: "This Month", value: fmtINR(thisMonthTotal), color: "text-yellow-400", icon: "📅" },
          { label: "Total Entries", value: expenses.length, color: "text-blue-400", icon: "📋" },
          { label: "Categories", value: catPieData.length + " active", color: "text-emerald-400", icon: "🏷️" },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="font-bold text-sm text-foreground mb-3">Category Breakdown</h3>
          {catPieData.length > 0 ? (
            <>
              <div className="h-36">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-5">
          <div className="flex flex-wrap gap-2 mb-4">
            <div className="relative flex-1 min-w-[160px]">
              ...
              <Input className="pl-9 h-8 text-[12px]" placeholder="Search expenses..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <SearchableSelect
          ...
          <div className="space-y-0 max-h-80 overflow-y-auto">
            {filtered.length === 0 ? (
              <p className="text-muted-foreground text-center py-10 text-sm">No expenses found</p>
            ) : (
              filtered.map(e => (
                <div key={e.id} className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
                  ...
                  <div className="flex items-center gap-2 ml-2 shrink-0">
                    ...
                    <button onClick={() => del.mutate(e.id)} className="p-1 rounded hover:bg-red-500/10 text-red-400"><Trash2 className="w-3 h-3" /></button>
                  </div>
                </div>
              ))
            )}
